...
> 着手順: M0→M1→M2→M3→M4→M5(完了) → **U1**→M5.1→M4.1→M5.2→D1→M6→D2
> **⚠ 2026-06-30: Besiege 大型アップデートにより MP が完全停止。U1(互換対応)を M5.1 前に挿入。**
...
| M4 | 弾体 MP 同期 | 2026-06-15 | ⚠ 実機確認未実施・同期是正残 → **M4.1**。U1 完了後に着手 |
| M5 | AdShootingProp 互換実装 + 追加機能 | 2026-06-17 | SP 実機確認の大半は済み。MP 系は U1 待ち → **M5.1 / M5.2** |
...
### U1: Besiege アップデート互換対応(M5.1 前の必須先行タスク)

> **2026-06-30 追加。Besiege 大型アップデートにより MP が完全停止。M5.1 以降の MP 観測ができない状態のため、先行して修正する。**

**概要**: アップデートで壊れた Modding API 互換を修正し、MP が再び動く状態に戻す。設計変更ではなく互換修正のみ。

**確認済み不具合**:

1. **🔴 MP クラッシュ(最優先)**: ホスト側で `Modding.ModNetworking.FragmentMessage` 内 NullRef。
   - スタック: `ProjectileSyncTransport` → `ModNetTransport.SendToAllOthers` → `ModNetworking.SendTo(Player)` → `FragmentMessage`
   - 推定: アップデートで `RPCDestination` または送信先プレイヤーのコネクション状態の扱いが変わった。
   - 調査起点: Cecil で新 DLL の `FragmentMessage` シグネチャ・`RPCDestination` 型を確認する(`docs/besiege-dll-inspect-cecil` 参照)。
   - 候補修正: プレイヤー送信前の null チェック追加、または新 API に合わせた呼び出し変更。

2. **🟡 エフェクト残留バグ**: 衝突後もエフェクトが消えない。アップデート前は正常だった。
   - 推定: `GameObject.Destroy` のタイミングまたはパーティクルシステムのライフサイクルが変わった。
   - 優先度はクラッシュより低いが、視覚的に目立つため U1 内で対処できれば対処する。

3. **⚪ 起動時 UInt64.Parse エラー**: `InternalModding.Mods.ModList+Mod.FromString` 内エラー。
   - ACMu のコードスタックに含まれないため Besiege 側の問題と考えられる。他 Mod を全無効にして再現するか確認できれば切り分け可能。ACMu 起因でなければ対処不要。

**受け入れ基準**:
- [ ] ホスト + クライアント 2 人で MP セッションを開始しシミュ開始してもクラッシュしない
- [ ] 弾を発射してもホスト側 NullRef が出ない
- [ ] エフェクトが衝突後に消える(または別 Issue として切り出す)
...
> **前提: U1 完了後に着手(MP が動かないと Issue #2 を観測できない)。**
>
> **2026-06-30 現在の進捗**: SP 可能な実機確認はほぼ消化済み。残りは MP 系のみ。
> - [x] カプセル衝突、ECannon PowerSlider、0.02s 遅延、Attaches — 確認済み
> - [ ] `useFreezingAttack` — 未確認(優先度低)
> - [ ] MP 系全般 — U1 後に確認
...
**受け入れ基準**:
- [ ] `PROGRESS.md` の M5 実機確認が全項目消化される(useFreezingAttack 以外)
