...
2. **依存規律**(ARCHITECTURE.md §1): 本体デコンパイル由来クラス
   ...
   ただし公式 Modding API(`Modding.*`)は安定APIとみなし Host / Weapons / PluginApi からも参照可(ARCHITECTURE.md §1-5)。
...
src/acmu/                プロジェクトルート(acmu.csproj はここ)
  ...
  ACMu.World/            IWorldFrame 実装・拡張EX(未作成。M6 で追加予定。現行 NullWorldFrame は ACMu.Host/Null/ に置かれる)
  ACMu.Weapons/          武装パイプライン・IProjectileService・武装ホスト
  ACMu.Compat/           旧ACM互換モジュール(PluginApi の利用者として書く)
  ACMu.Host/             ACMUcore・配線・Null実装スタブ(NullWorldFrame 等)
...
- ビルド: `./build.sh`(中身: VS MSBuild を vswhere で検索して実行、`dotnet msbuild` は .NET 3.5 Unity プロファイル非対応のため不使用 → PostBuildEvent で `ACMu/acmu.dll` へコピー → `$BESIEGE_DIR/Besiege_Data/Mods/ACMu/` へコピー)
...
| ブロックの値読み取り | `IAcmuServices.Blocks.FromGameObject(go)` → `IBlockAccessor`(BlockBehaviour を直接触らない) |
