...
1. **契約凍結**: `src/ACMu.Core/` と `src/ACMu.PluginApi/` は読み取り専用。
   変更が必要だと感じたら作業を止め、「契約変更が必要: <理由>」とだけ報告して終了する。
2. **依存規律**(ARCHITECTURE.md §1): 本体デコンパイル由来クラス
   ...
   `using`/参照してよいのは `src/ACMu.Adapter/` のみ。実装層同士の横参照禁止(Core契約経由のみ)。
...
```
src/ACMu.Core/      契約のみ(凍結)
src/ACMu.PluginApi/ 公開面(凍結)
src/ACMu.Adapter/   本体依存の隔離実装
src/ACMu.Net/       INetworkTransport 実装
src/ACMu.World/     IWorldFrame 実装・拡張EX
src/ACMu.Weapons/   武装パイプライン・IProjectileService・武装ホスト
src/ACMu.Compat/    旧ACM互換モジュール(PluginApi の利用者として書く)
src/ACMu.Host/      ModEntryPoint・ACMUcore・配線
docs/               制約・設計ドキュメント(読み取り専用)
ACMu.csproj         単一プロジェクト(全 src を含む)
mod.json            Besiege Mod マニフェスト
...
- ログは入れすぎない。初期化・エラー・ネットワーク異常のみ。毎フレームログ禁止

## 3. ビルドと検証

- ビルド: `./build.sh`(中身: `dotnet build -c Release` → `$BESIEGE_DIR/Besiege_Data/Mods/ACMu/` へコピー)
...
| ホスト/クライアント判定 | `IGameSessionInfo.IsHost / IsClient`(StatMaster を直接見ない) |
