...
>
> **凍結の条件付き解除**: 凍結を守るために設計が明らかに複雑・無意味になる場合は Core/PluginApi を変える方が正。
> その場合は勝手に変えず、オーナーに「何をどう変えるか/影響範囲/既存利用箇所・契約不変条件への波及がないか」を説明し、
> **同意を得られた時のみ**変更する。同意なしの変更は禁止(CLAUDE.md §0-1 と同一規定)。
